    ...
    if len(os.listdir(path+'/Data/New/'))!=0:
        ...
        for filename in os.listdir(path+'/Data/New/'):
            with open(os.path.join(path+'/Data/New/', filename), 'r') as f:
                combined_regions_df[i]=pd.concat([combined_regions_df[i],pd.read_csv(f)],axis=0,ignore_index=False)
                i+=1
        for i in range(len(combined_regions_df)):
            combined_regions_df[i]=combined_regions_df[i].set_index('UTC Time at End of Hour')
...
# def prophet(combined_regions_df,i):
#     models = []
...
# def pforecast(number_of_days,p_models,type,region):
#     if type == 'Demand':
#         future_demand = p_models[0].make_future_dataframe(periods=number_of_days)
#         forecast = p_models[0].predict(future_demand)
#         #p_models[regions_dict[region]].plot(forecast)
#         return forecast[['ds','yhat']].tail(number_of_days+1)
#     elif type == 'Net Generation':
#         future_net = p_models[1].make_future_dataframe(periods=number_of_days)
#         forecast = p_models[1].predict(future_net)
#         return forecast[['ds','yhat']].tail(number_of_days+1)
...
def plotss(combined_regions_df,predictions,region,name):
    ...
    for i,regions in enumerate(regions_list):
        regions_dict[regions] = i
    fig = px.line(combined_regions_df[regions_dict[region]], x=combined_regions_df[regions_dict[region]].index , y=name+" (MW)",title=region)
...
def prediction(region):
    combined_regions_df = preprocessing()
    print(region)
    # i=regions_dict[region]
...
def prophet(combined_regions_df):
  ...
  for i in range(0,13):
    ...
    # exec("pickle.dump(" + filename1 + ", open(" + filename1 + ".sav, 'wb'))")
    # exec("pickle.dump(" + filename2 + ", open(" + filename2 + ".sav, 'wb'))")
